...
def _make_api_call(tool_name=None, **kwargs):
    """Helper function to make API calls with consistent structure.

    Args:
        tool_name: Explicit primitive tool name to call. If None, infers from the
                   caller's function name (used by the primitive functions themselves).
                   Composite tools MUST pass tool_name explicitly.
    """
    if tool_name is None:
        tool_name = inspect.currentframe().f_back.f_code.co_name
    headers = {"Content-Type": "application/json"}
    url = f"{tools_url}/{tool_name}"
    response = requests.post(url, json={"name": tool_name, "arguments": kwargs}, headers=headers)
