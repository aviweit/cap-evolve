"""The ``runtime:`` spec field — validation, resolution, and preflight refusals.

Everything here is OFFLINE: no service is started and no network is touched. The one
thing that must never regress is the *refusal* behaviour — a spec whose runtime cannot
...
from cap_evolve import runtime as rt
...
def test_absent_runtime_is_direct():
    assert rt.declared({}) == rt.DIRECT
    assert rt.declared({"runtime": None}) == rt.DIRECT
    assert rt.declared({"runtime": ""}) == rt.DIRECT


def test_known_values_are_normalised():
    assert rt.declared({"runtime": "spa"}) == rt.SPA
    assert rt.declared({"runtime": "  SPA "}) == rt.SPA
    assert rt.declared({"runtime": "direct"}) == rt.DIRECT
...
def test_unknown_value_is_refused_by_name():
    ...
    with pytest.raises(rt.RuntimeError_) as e:
        rt.declared({"runtime": "sap"})
...
def test_spa_runtime_skill_is_registered():
    row = MANIFEST.get("spa")
    assert row, "the spa runtime skill is missing from the manifest"
    assert row["component"] == "runtime"
...
def test_skill_dir_rejects_a_component_mismatch():
    ...
    hand the runtime layer the wrong skill."""
    fake = {"spa": {"component": "capability", "path": "capabilities/skill-package"}}
    with pytest.raises(rt.RuntimeError_) as e:
        rt.skill_dir("spa", fake, SKILLS)
    assert "not 'runtime'" in str(e.value)


def test_skill_dir_reports_an_unregistered_runtime():
    with pytest.raises(rt.RuntimeError_) as e:
        rt.skill_dir("nope", {}, SKILLS)
...
def test_direct_preflight_is_a_noop():
    assert rt.preflight({}, {}, SKILLS) == {"runtime": rt.DIRECT}
...
def test_spa_preflight_refuses_when_not_provisioned(tmp_path, monkeypatch):
    ...
    with pytest.raises(rt.RuntimeError_) as e:
        rt.preflight({"runtime": "spa", "skill_name": "x"}, MANIFEST, SKILLS)
    msg = str(e.value)
...
def test_spa_preflight_needs_a_skill_name(tmp_path, monkeypatch):
    ...
    with pytest.raises(rt.RuntimeError_):
        rt.preflight({"runtime": "spa"}, MANIFEST, SKILLS)
...
def test_describe_direct_and_spa():
    assert "direct" in rt.describe({"runtime": "direct"})
    line = rt.describe({"runtime": "spa", "store_port": "8000", "spa_port": "7000",
