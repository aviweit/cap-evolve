...
def test_the_adapter_is_sourced_from_the_example_not_duplicated_into_templates():
    """Each arm's adapter carries its own scoring/feedback stack, and the two were already 572
    lines identical before #479 shared them. A copy under templates/adapters/ would re-create
    that duplication and could drift from the example a reviewer actually reads."""
    case = _arm_case()
...
def test_the_spa_arm_bounds_its_service_logs():
    """spa_env opens store.log / proxy-agent.log in APPEND mode and never rotates them, and the
    proxy-agent log reaches 100MB+ within a run (it records every ChatRequest). The stack's
    vendor dir is CACHED between jobs so provisioning is not repeated, which means an unrotated
    log grows for the life of the self-hosted runner and fills its disk for a reason unrelated
    to the benchmark. Truncate per leg; keep a bounded tail for the reviewer."""
    case = _arm_case()
    spa = case.split('if [ "$ARM" = "direct" ]', 1)[1].split("else", 1)[1]
    assert "SPA_VENDOR_DIR" in spa, "the run must agree with ci_setup.sh on the vendor dir"
    assert ': > "$_log"' in spa, "logs are not truncated at leg start"
    assert "tail -c" in spa and ".tail.log" in spa, "no bounded tail captured for the artifacts"
