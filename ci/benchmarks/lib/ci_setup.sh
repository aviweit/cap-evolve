...
case "$BENCH" in
  ...
  skillberry_tau2_direct|skillberry_tau2_spa)
    # A DIFFERENT tau2 build from the `tau2` leg above, and deliberately so. Both arms are
    # onboarded against skillberry-ai/skillberry-benchmarks at a PINNED commit, whose tau2
    # runner carries what SPA delivery needs (context headers on the agent's LLM calls, the
    # proxy-side trajectory merged into tau2's own, a disconnect at session end) while still
    # exposing the plain `airline` domain the direct arm uses. ONE build for both arms is what
    # keeps a direct-vs-spa comparison meaningful; the public sierra-research checkout has
    # neither the `airline_skillberry` domain nor the [skillberry] extra.
    #
    # The pin is the SAME default the arms' own setup.sh scripts use, so a CI number and a
    # local `bash examples/.../run.sh` number refer to the same benchmark code. Keep them in
    # step: changing one and not the other silently makes the two incomparable.
    ...
    if [ "$BENCH" = "skillberry_tau2_spa" ]; then
      ...
      # The flip side of caching: spa_env opens store.log / proxy-agent.log in APPEND mode and
      # never rotates them, so a cached vendor dir means they grow for the life of the runner.
      # These reach 100MB+ quickly on a real run. Truncate anything already oversized here, and
      # run_suite.sh truncates again per leg — steady-state disk is then one leg's worth of log,
      # not every leg since the runner was provisioned.
      for _log in "$SPA_VENDOR_DIR/skillberry-agent/proxy-agent.log" \
                  "$SPA_VENDOR_DIR/skillberry-store/store.log"; do
        [ -f "$_log" ] || continue
        _sz=$(stat -c %s "$_log" 2>/dev/null || echo 0)
        if [ "$_sz" -gt 20971520 ]; then          # 20MB
          : > "$_log"
          echo "truncated oversized $(basename "$_log") (was $((_sz / 1048576))MB)"
        fi
      done
      # PROVISION ONLY (clone + venv + install the Store and the Proxy-Agent), never start:
      # starting belongs to the run, exactly as in the arm's own setup.sh. spa_env is the same
      # module the example uses, so CI and a local run provision an identical stack.
